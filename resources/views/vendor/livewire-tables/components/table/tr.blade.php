...
<tr rowpk='{{ $row->{$primaryKey} }}' x-on:dragstart.self="currentlyReorderingStatus && dragStart(event)"
    ...
    @if ($this->hasDisplayLoadingPlaceholder()) wire:loading.class.add="hidden d-none"
    @else
        wire:loading.class.delay="opacity-50 dark:bg-gray-900 dark:opacity-60" @endif
    id="{{ $tableName }}-row-{{ $row->{$primaryKey} }}" :draggable="currentlyReorderingStatus"
    ...
    {{ $attributes->merge($customAttributes)->class([
            ...
            'bg-gray-50 dark:bg-gray-800 dark:text-white rappasoft-striped-row' =>
                $isTailwind && ($customAttributes['default'] ?? true) && $rowIndex % 2 !== 0,
        ])->class(['cursor-pointer' => $isTailwind && $this->hasTableRowUrl() && ($customAttributes['default'] ?? true)])->class(['rappasoft-striped-row' => $isBootstrap && $rowIndex % 2 === 0 && ($customAttributes['default'] ?? true)])->class(['rappasoft-striped-row' => $isBootstrap && $rowIndex % 2 !== 0 && ($customAttributes['default'] ?? true)])->except(['default', 'default-styling', 'default-colors']) }}>
    {{ $slot }}
</tr>

@if ($logged_in_user->hasTwoFactorEnabled())
    ...
    <a href="{{ route('frontend.auth.account.2fa.disable') }}"
        class="btn btn-danger btn-sm btn-block">@lang('Remove Two Factor Authentication')</a>
...
@else
    ...
    <a href="{{ route('frontend.auth.account.2fa.create') }}"
        class="btn btn-success btn-sm btn-block">@lang('Enable Two Factor Authentication')</a>
@endif

@if ($user->trashed() && $logged_in_user->hasAllAccess())
    ...
    @if (config('boilerplate.access.user.permanently_delete'))
        <x-utils.delete-button :href="route('dashboard.auth.user.permanently-delete', $user)" :text="__('Permanently Delete')" />
    @endif
@else
    ...
    @if (!$user->isActive())
        <x-utils.form-button :action="route('dashboard.auth.user.mark', [$user, 1])" method="patch" button-class="btn btn-primary btn-sm" icon="fas fa-sync-alt"
    ...
    @if ($user->id !== $logged_in_user->id && !$user->isMasterAdmin() && $logged_in_user->hasAllAccess())
        <x-utils.delete-button :href="route('dashboard.auth.user.destroy', $user)" />
    @endif
    ...
    @if ($user->isMasterAdmin() && $logged_in_user->isMasterAdmin())
        <div class="dropdown d-inline-block">
            ...
            <div class="dropdown-menu" aria-labelledby="moreMenuLink">
                <x-utils.link :href="route('dashboard.auth.user.change-password', $user)" class="dropdown-item" :text="__('Change Password')"
        ...
        <div class="dropdown d-inline-block">
            ...
            <div class="dropdown-menu" aria-labelledby="moreMenuLink">
                ...
                @if ($user->id !== $logged_in_user->id && !$user->isMasterAdmin())
                    <x-utils.form-button :action="route('dashboard.auth.user.clear-session', $user)" name="confirm-item" button-class="dropdown-item"
                    ...
                    <x-utils.form-button :action="route('dashboard.auth.user.mark', [$user, 0])" method="patch" name="confirm-item"

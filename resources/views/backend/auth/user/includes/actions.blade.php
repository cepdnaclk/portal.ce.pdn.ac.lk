<div class="d-flex align-items-center flex-row gap-1" role="group" aria-label="User Management Actions">
    ...
    @else
        ...
        @if (!$user->isActive())
            <x-utils.form-button :action="route('dashboard.auth.user.mark', [$user, 1])" method="patch" button-class="btn btn-primary btn-sm text-nowrap"
        ...
        @if ($user->id !== $logged_in_user->id && !$user->isMasterAdmin() && $logged_in_user->hasAllAccess())
            <x-utils.delete-button :href="route('dashboard.auth.user.destroy', $user)" :text="__('')" />
        @endif

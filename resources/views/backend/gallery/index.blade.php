        ...
        <x-backend.card>
            ...
            <x-slot name="headerActions">
                <x-utils.link
                    ...
                    :href="route('dashboard.' . $type . '.edit', $imageable)"
                    :text="__('Back to Edit')" />
            </x-slot>
            ...
            <x-slot name="body">
                    ...
                    <div class="mb-4">
                        ...
                        <form id="uploadForm" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                <label for="images">Select Images (Max {{ $stats['max_images'] - $stats['total_images'] }} more)</label>
                                <input 
                                    type="file" 
                                    class="form-control" 
                                    id="images" 
                                ...
                                <small class="form-text text-muted">
                                    Only JPEG images are allowed. Maximum size: {{ config('gallery.max_file_size') / 1024 }}MB per image.
                                    Minimum dimensions: {{ config('gallery.min_width') }}x{{ config('gallery.min_height') }} pixels.
                                </small>
                            </div>
                ...
                @else
                    <div class="alert alert-warning">
                        Maximum number of images ({{ $stats['max_images'] }}) reached. Please delete some images before uploading more.
                    </div>
                @endif
                ...
                <div class="mb-4">
                    ...
                    <div id="galleryContainer" class="row g-3">
                        @forelse($imageable->gallery as $image)
                            <div class="col-md-4 gallery-item" data-id="{{ $image->id }}" data-order="{{ $image->order }}">
                                <div class="card h-100">
                                    <img src="{{ $image->getSizeUrl('thumb') }}" class="card-img-top" alt="{{ $image->alt_text }}">
                                    <div class="card-body">
                                        @if($image->is_cover)
                                            <span class="badge bg-success mb-2">Cover Image</span>
                                        @else
                                            <button type="button" class="btn btn-sm btn-outline-success set-cover-btn mb-2" data-id="{{ $image->id }}">
                                                Set as Cover
                                            </button>
                                        @endif
                                        ...
                                        <div class="form-group mt-2">
                                            <label>Caption</label>
                                            <textarea class="form-control form-control-sm image-caption" 
                                                      data-id="{{ $image->id }}" 
                                                      rows="2"
                                        ...
                                        <div class="mt-3 d-flex justify-content-between">
                                            <button type="button" class="btn btn-sm btn-primary save-metadata-btn" data-id="{{ $image->id }}">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.save-metadata-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                const altText = document.querySelector(`.image-alt[data-id="${imageId}"]`).value;
                const caption = document.querySelector(`.image-caption[data-id="${imageId}"]`).value;
                const credit = document.querySelector(`.image-credit[data-id="${imageId}"]`).value;
                ...
                fetch(updateRoute.replace(':id', imageId), {
                    ...
                    body: JSON.stringify({ alt_text: altText, caption: caption, credit: credit })
                })
                .then(response => response.json())
                .then(data => {
                    alert(data.message);
